//
//  NoticeConst.h
//  TokenWalletProject
//
//  Created by mnz on 2022/3/17.
//  Copyright © 2022 Zinkham. All rights reserved.
//

static NSString * const kNetworkAvailableNotification = @"kNetworkAvailableNotification";
static NSString * const kNetworkDisabledNotification = @"kNetworkDisabledNotification";

static NSString * const kChainNodeUpdateNotification = @"kChainNodeUpdateNotification";

static NSString * const kChangeWalletNotification = @"kChangeWalletNotification";

static NSString * const kRefreshCoinListNotification = @"RefreshCoinList_Notification";

static NSString * const kReloadWalletNotification = @"ReloadWalletNotification";

static NSString * const kRedRoseGreenFellNotification = @"RedRoseGreenFellNotification";
