    ...
    if (!self.singleLine && self.preferredMaxLayoutWidth > 0) {
        ...
        for (UIView *view in subviews) {
            ...
            if (previousView) {
                CGFloat width = size.width;
                currentX += itemSpacing;
                if (currentX + width <= self.preferredMaxLayoutWidth) {
                    if (CGRectGetMaxY(previousView.frame) < self.bounds.size.height) {
...
- (void)changeTagAtIndex:(NSInteger)index{
    SKTagButton *btn = self.subviews[index];
    btn.alpha = 0.1;
    self.didSetup = NO;
}
