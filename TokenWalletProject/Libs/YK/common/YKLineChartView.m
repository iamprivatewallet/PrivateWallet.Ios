...
-(void)layoutSubviews{
    ...
    for (NSInteger i = _yLinesViews.count-1; i >= 0 ;i--){
        ...
        line.frame = CGRectMake(0, yStart, self.bounds.size.width, .5);
        yStart += lineHeight;
        yLineStartx = line.yLineStartx;
    }

    _contentScrollView.frame = CGRectMake(yLineStartx, 0, self.bounds.size.width - yLineStartx, _chartYLength + 70);
    ...
    _xlineView.frame = CGRectMake(yLineStartx, 0, 1, yStart-_topSpaceLength+3);
    _xlineView.backgroundColor = _uiConfig.ylineColor;
...
-(void)emptyCharts{
    ...
    [_xlineView removeFromSuperview];
...
-(void)installYLines{
    ...
    _xlineView.backgroundColor = [UIColor whiteColor];
    [self addSubview:_xlineView];
    
    double  yEachValue = (_dataSource.max-_dataSource.min)/(_numberOfYLines - 1);
    for (NSInteger i = 0; i < _numberOfYLines ; i++) {
        YKYLineView *line = [YKYLineView new];
        CGFloat yValue = _dataSource.min+yEachValue*i;
        if (i == 0) {
            line.ishidden = NO;
        }else{
            line.ishidden = YES;
        }
        [line setYValue:[CATCommon DecimalDigits:[NSString stringWithFormat:@"%f",yValue] Scale:3] suffix:_dataSource.ySuffix withUIConfgi:_uiConfig];
...
-(void)installDrawlines{
    for (NSInteger i = 0 ; i < _dataSource.showNumbers.count; i ++){
        ...
        //计算 y 的位置
        CGFloat percentage = (1 - fabs(number-_dataSource.min)/fabs(_dataSource.max - _dataSource.min));
