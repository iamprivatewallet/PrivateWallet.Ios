...
- (NSArray *)jq_lookupTable:(NSString *)tableName dicOrModel:(id)parameters whereFormat:(NSString *)format, ...
{
    ...
    __block NSArray *clomnArr = nil;
    [self jq_inDatabase:^{
        clomnArr = [self getColumnArr:tableName db:self->_db];
    }];
...
- (NSString *)jq_tableItemMax:(NSString *)tableName column_name:(NSString *)column_name {
    NSString *sqlstr = [NSString stringWithFormat:@"SELECT MAX(%@) as 'count' FROM %@", column_name, tableName];
    FMResultSet *set = [_db executeQuery:sqlstr];
    while ([set next])
    {
        return [set stringForColumn:@"count"];
    }
    return @"0";
}
