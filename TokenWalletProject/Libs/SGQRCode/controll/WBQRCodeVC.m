...
- (void)setupQRCodeScan {
    ...
    [obtain setBlockWithQRCodeObtainScanResult:^(SGQRCodeObtain *obtain, NSString *result) {
        if (result) {
            ...
            if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(scanResult:)]) {
                [weakSelf.delegate scanResult:result];
                [weakSelf.navigationController popViewControllerAnimated:YES];
            }
            if (self.delegate && [self.delegate respondsToSelector:@selector(scanNoPopWithResult:)]) {
                [weakSelf.delegate scanNoPopWithResult:result];
            }
        }
    }];
}
